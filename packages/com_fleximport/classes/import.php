			...
			switch ($this->_params->get('localisation')) {
				case "local":
					...
					foreach ($filesList as $fileToImport) {
						if (preg_match('/' . $importFilename . '/i', $fileToImport)) {
				...
				case "ftp":
					...
					// connection au FTP
					$clientFTP = JClientFtp::getInstance($importServer, $importPort, array(), $importUsername, $importPassword);
				...
				switch ($extension) {
					...
					case 'zip':
						...
						if ($zip->open($this->_pathToTemp . $file) === true) {
							...
							if (($zipIndex = $zip->locateName($fileToFound, ZIPARCHIVE::FL_NOCASE)) !== false) {
                                //$zip->renameIndex($zipIndex,$fileToFound);
								$archiveValid = true;
			...
			for ($i = $stepAjax ; $i <= $totalRaw ;$i++) {
				...
				if ($dataAction['action']) {
					...
					foreach ($fields as &$field) {
						...
						if ((!$fieldValues || $fieldImport->_fieldParams->get('categories_keep_during_update')) && ($dataAction['action'] == 2 && $dataAction['id']))
		...
		for($i = $stepAjax; $nbData > $i ;$i++) {
			...
			switch ($actionType) {
				case 1: // ajout
					...
					$post['jform']['access'] = $this->_params->get("access", "1");
