...
class fleximportPluginpublishup extends fleximportFieldPlugin {
    public function formatValues()
    {
        $separator = $this->_fieldParams->get('publishup_separator', '/');
        $dayPos = (int)$this->_fieldParams->get('publishup_day', 2);
        $monthPos = (int)$this->_fieldParams->get('publishup_month', 1);
        $yearPos = (int)$this->_fieldParams->get('publishup_year', 0);
        $hour = $this->_fieldParams->get('publishup_hour', '23:59:59');
    ...
    {
        $post['jform']['publish_up'] = $this->_fieldValues;
    ...
    public function getFlexicontentValues($itemID = 0)
    {
    	...
    	$query->select('publish_up')->from('#__content')->where('id='.(int)$itemID);
    	$db->setQuery($query);
