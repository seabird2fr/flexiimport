...
class fleximportPlugincategories extends fleximportFieldPlugin {
    ...
    {
        // si on a g�rer les valeurs par d�faut, on a tableau multidimensionnel , il faut le r�duire au premier niveau
        ...
        // s'il faut spliter les valeurs des cat�gories avant de traiter la liaison
        if ($catSplit) {
        ...
        if ($catForceDefault && count($categories)) {
            ...
            // force la valeur par d�faut si aucune cat�gorie correspond aux cat�gories existantes
            $categories = $this->getDefaultValue();
        }

        $this->_fieldValues = array_unique($categories);
