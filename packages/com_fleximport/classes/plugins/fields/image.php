...
class fleximportPluginimage extends fleximportFieldPlugin {
	...
	{
		$imageSeparator = $this->_fieldParams->get('image_separator', '::');
		$imageExport = $this->_fieldParams->get('image_export', '1');
		foreach ($this->_fieldValues as &$value) {
			...
			@list($imgFile, $imgTitle, $ImgDesc, $imgAlt) = explode($imageSeparator, $value);
			if ($imageExport <= '3') $imgAlt = '';
			if ($imageExport <= '2') $ImgDesc = '';
			if ($imageExport <= '1') $imgTitle = '';
			...
			if ($imgFile) {
				...
				if ($fileExist && $fileExistMethod == "1") {
					$value['originalname'] = $filenameSanitize;
					$value['existingname'] = $filenameSanitize;
					$value['alt'] = $imgAlt;
					$value['title'] = $imgTitle;
					$value['desc'] = $ImgDesc;
				} else {
	...
	public function formatValuesExport()
	{
		...
		if (JFactory::getApplication()->input->get('params_export_attachment')=='2' and count($this->_fieldValues)) {
			...
			foreach ($this->_fieldValues as $idv => $fieldValue) {
				$fileOk = false;
				$fileID = $this->fileExist($fieldValue['originalname']);
				if ($fieldInfo = $this->fileInfo($fileID)) {
					if ($fieldInfo->url) {
		...
		foreach ($this->_fieldValues as $idv => $fieldValue) {
			if ($imageExport == 4) {
				$this->_fieldValues[$idv] = $fieldValue['originalname'] . $imageSeparator . $fieldValue['title'] . $imageSeparator . $fieldValue['desc'] . $imageSeparator . $fieldValue['alt'];
			} elseif ($imageExport == 3) {
				$this->_fieldValues[$idv] = $fieldValue['originalname'] . $imageSeparator . $fieldValue['title'] . $imageSeparator . $fieldValue['desc'];
			} elseif ($imageExport == 2) {
				$this->_fieldValues[$idv] = $fieldValue['originalname'] . $imageSeparator . $fieldValue['title'];
			} else {
				$this->_fieldValues[$idv] = $fieldValue['originalname'];
			}
		}
	}
	/*
