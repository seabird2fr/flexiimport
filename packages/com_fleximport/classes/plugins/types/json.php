...
class fleximportPluginType extends fleximportPluginTypeBase {
	...
	*/
    private function getValuesJSON($index = 0, $cleJSON = array())
    {
        ...
        foreach ($cleJSON as $cle) {
            $tmpValue = null;
            if (is_object($data)&&isset($data->$cle)){
                $tmpValue = $data->$cle;
            }else if(is_array($data)&& isset($data[$cle])){
                $tmpValue = $data[$cle];

            }
            if (!is_null($tmpValue)){
                if (is_array($tmpValue)){
                    $fieldValues = array_merge($fieldValues,$tmpValue);
                }else{
	...
	*/
    public function exportData($datas = array(), $type = 0)
    {
        ...
        if ($f = fopen($this->_pathToTemp . $this->_filename, 'wb')) {
            fwrite($f, json_encode($lineValues,JSON_UNESCAPED_UNICODE));
            fclose($f);
